...
{
    internal class HashSetSerializer : ISerializerComponent
    {
        public bool Serialize(Type _Type, object _Object, out byte[] _Bytes)
        {
            if (_Type.IsGenericType && _Type.GetGenericTypeDefinition() == typeof(HashSet<>))
            {
        ...
        public bool DeSerialize(Type _Type, byte[] _Bytes, out object _Object)
        {
            if(_Type.IsGenericType && _Type.GetGenericTypeDefinition() == typeof(HashSet<>))
            {
                Type var_HashSetType = typeof(HashSet<>);
                Type var_GenericItemType = _Type.GetGenericArguments()[0];
                Type var_GenericHashSetType = var_HashSetType.MakeGenericType(var_GenericItemType);
                ...
                _Object = Activator.CreateInstance(var_GenericHashSetType, var_Items);
                return true;
            }
